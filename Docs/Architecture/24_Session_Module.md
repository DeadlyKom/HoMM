...
Это означает, что на фазе загрузки карта рассматривается как совокупность не только layout-данных, но и metadata и графических пакетов.
...
`Session` — это preparatory runtime module высокого значения. Он не просто загружает карту, а собирает игровую сессию из metadata, графики, data blocks, save/load механики и служебной инфраструктуры. Именно он превращает абстрактный проект в конкретную партию, которая затем передаётся миру.
