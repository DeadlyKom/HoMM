...
## Перемещаемый код как часть общей asset-модели

Особенно сильный момент проекта в том, что code-assets здесь не просто “лежат на диске и потом копируются в память”.

Для крупных модулей Builder заранее готовит relocatable-формат:
- модуль собирается дважды с разными `ORG`;
- разница между двумя бинарниками превращается в таблицу релокации;
- в начало payload'а добавляется маленький корректировщик адресов.

За счёт этого runtime получает очень дешёвую схему исполнения:
- asset загружается через общий `AssetsManager`;
- при первом входе сам поправляет адресозависимые слова;
- затем передаёт управление уже в рабочее тело модуля.

То есть и здесь проект остаётся верен общей идее:
тяжёлая интеллектуальная работа выполняется на build-time, а runtime получает компактный и уже подготовленный к потреблению payload.
...
Когда код, графика, метаданные и текст уже живут как assets, проекту проще расти без полного пересмотра модели поставки.
