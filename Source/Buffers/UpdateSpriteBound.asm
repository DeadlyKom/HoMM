...
; -----------------------------------------
SpriteBound:    ; проверка наличия bound спрайта
                LD A, C
                OR A
                RET Z                                                           ; выход, если высота bound спрайта нулевая
                OR B
                RET Z                                                           ; выход, если ширина bound спрайта нулевая
                ; -----------------------------------------
                ...
                JR Z, .LastHex                                                  ; переход, если спрайт на границе гексагона, необходимо
                                                                                ; обновить нижний гексагон

                ; выравнивание позиции спрайта по следующей границе гексагона
                ADD A, D        ; позиция спрайта в пикселях по оси Y
...
.LastHex        ; переход на следующюю позицию гексагона
                LD A, D         ; позиция спрайта в пикселях по оси Y
                ADD A, HEXTILE_BASE_SIZE_Y << 3
                LD D, A

.UpdateRow      ; округление до знакоместа
...
.Loop           ; следующий элемент списка отображения
                ...
                ; получение номер линии с учётом пропускаемых знакоместныйх строк
                LD A, (IX + 4)                                                  ; количество пропускаемых знакомест (спрайт находится ниже экрана)
                ADD A, A    ; x2
                ADD A, A    ; x4
                ADD A, A    ; x8
                ADD A, (IX + 3)                                                 ; номер строки по вертикали в пикселях
...
.Found          EX AF, AF'
                ...
                LD A, E             ; количество доступных колонок
                EX AF, AF'
                ...
                BIT 7, D
                JR Z, .L1
                LD A, #82       ; 7 бит флаг чётности, 2 первый рисуемый столбец
                                ; см. генерацию списка отображения
                SUB D
                JR C, .L1
                INC L
