...
; -----------------------------------------
; ограниченный поиск пути в ширину (Red Blob Games: Obstacles)
; In:
;   DE' - начальные координаты (D - y, E - x)
;   BC' - координаты назначения (B - y, C - x)
; Out:
;   A'  - длина найденного пути (1..PATHFINDING_MAX_DISTANCE),
;         0, если цель недостижима в пределах ограничения;
;         для дальней цели возвращается локальный префикс ровно из 8 шагов
;   Adr.SortBuffer - FPath от цели к старту, без стартового гекса
; Corrupt:
;   HL, DE, BC, AF, AF'
; Note:
;   IX и IY сохраняются для последующего Character.PathInitialize;
;   код и рабочая область расположены в странице 1;
...
.InitSearch
                ...
                ; очистить локальную карту родителей значением "не посещён"
                LD HL, Adr.PathfindingParents
                LD (HL), PATHFINDING_CELL_UNVISITED
                LD DE, Adr.PathfindingParents + 1
                LD BC, Size.PathfindingParents - 1
                LDIR

                ; центр локальной карты является стартовой клеткой
                LD HL, Adr.PathfindingParents + PATHFINDING_MAX_DISTANCE * PATHFINDING_RANGE_DIAMETER + PATHFINDING_MAX_DISTANCE
                LD (HL), PATHFINDING_CELL_START

                ; первый элемент FIFO: dq=0, dr=0, depth=0
                LD HL, Adr.PathfindingQueue
                LD (.HeadPtr), HL
                XOR A
                LD (HL), A
                INC HL
...
; Out:
;   HL - адрес элемента Adr.PathfindingParents
; Corrupt:
;   DE, HL, AF
; -----------------------------------------
.GetParentAdr   LD A, B
...
.BestQ          DB #00
.BestR          DB #00
.HeadPtr        DW Adr.PathfindingQueue
.TailPtr        DW Adr.PathfindingQueue
