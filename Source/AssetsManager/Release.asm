
                ifndef _ASSETS_MANAGER_RELEASE_
                define _ASSETS_MANAGER_RELEASE_
; -----------------------------------------
; освободить ранее загруженный ресурс
; In:
;   A  - идентификатор ресурса
; Out:
;   HL - адрес загрузки/распаковки
;   IX - адрес структуры FAssets
; Corrupt:
;   HL, E, BC, AF, IX
; Note:
;   ℹ️ необходимо включить страницу с данными о доступной ОЗУ
; -----------------------------------------
ReleaseAsset:   ; расчёт адреса информации о ресурсе
                ASSETS_ADR_A                                                    ; расчёт адреса информации о ресурсе
                
                ; проверить необходимость загрузки ресурса
                BIT ASSETS_LOAD_BIT, (IX + FAssets.Flags)
                RET Z                                                           ; выход, если ресурс не загружен

                RES ASSETS_LOAD_BIT, (IX + FAssets.Flags)                       ; сброс флага загрузки ресурса
                SET ASSETS_MARKED_BIT, (IX + FAssets.Address.Page)              ; установка флага невостребованный
                SET ASSETS_ALLOCATION_BIT, (IX + FAssets.Address.Page)          ; установка флага, требуется автоматическая аллокация
; -----------------------------------------
; освободить ранее занятую область данными в доступной ОЗУ
; In:
;   IX - адрес структуры FAssets
; Out:
; Corrupt:
;   HL, E, BC, AF
; Note:
;   ℹ️ необходимо включить страницу с данными о доступной ОЗУ
; -----------------------------------------
Release:        ; -----------------------------------------
                ; преобразование размера ресурса в блоки по 256 байт
                ; In:
                ;   IX - адрес структуры FAssets
                ; Out:
                ;   A  - количество блоков по 256 байт
                ;   BC - реальный размер блока до 16кб
                ; Corrupt:
                ;   BC, AF
                ; -----------------------------------------
                CALL CalcSizeToBlock
                LD E, A                                                         ; сохранение количество блоков по 256 байт

                ; корректировка количества занимаемых блоков (если адрес не выровнен)
                XOR A
                CP (IX + FAssets.Address.Adr.Low)
                ADC A, E
                LD E, A                                                         ; сохранение количества блоков по 256 байт (выровненый)

                ; -----------------------------------------
                ; преобразование линейного адреса в расположение и номер бита
                ; In:
                ;   IX - адрес структуры FAssets
                ; Out:
                ;   HL - адрес расположения бита линейного адреса
                ;   B  - номер бита линейного адреса (0..7)
                ; Corrupt:
                ;   HL, BC, AF
                ; -----------------------------------------  
                CALL CalcAvailableMem
                LD C, E                                                         ; восстановление количества блоков по 256 байт (выровненый)
; -----------------------------------------
; очистить выделенную область памяти в доступной ОЗУ
; In:
;   L - смещение расположения бита линейного адреса
;   C - количество блоков по 256 байт
;   B - номер бита линейного адреса (0..7)
; Out:
; Corrupt:
;   HL, BC, AF
; Note:
;   ℹ️ необходимо включить страницу с данными о доступной ОЗУ
; -----------------------------------------
.Claer          ; преобразование номера бита в код инструкции сброса
                LD A, B
                ADD A, A    ; x2
                ADD A, A    ; x4
                ADD A, A    ; x8
                OR #86                                                          ; RES n, (HL)
                LD (.BIT), A

                ; сброс бита первого блока выделенной области
                LD H, HIGH Adr.InitialBlockMem
.BIT            EQU $+1
                DB #CB, #00
; -----------------------------------------
; очистить занятую область памяти в доступной ОЗУ (принудительно)
; In:
;   L - смещение расположения бита линейного адреса
;   C - количество блоков по 256 байт
;   B - номер бита линейного адреса (0..7)
; Out:
; Corrupt:
;   HL, BC, AF
; Note:
;   ℹ️ необходимо включить страницу с данными о доступной ОЗУ
; -----------------------------------------
ClaerForce:     LD H, HIGH Adr.AvailableMem
.Loop           LD A, B
                ADD A, A    ; x2
                ADD A, A    ; x4
                ADD A, A    ; x8
                OR #86                                                          ; RES n, (HL)
                LD (.BIT), A
.BIT            EQU $+1
                DB #CB, #00

                DEC C
                RET Z                                                           ; выход, если счётчик блоков 256 байт обнулён

                ; расчёт номера следующего бита в текущем байте
                DEC B
                JP P, .Loop                                                     ; переход, если в текущем байте остались неочищенные биты

                ; подготовка обработки следующего байта битовой карты
                LD B, #07
                INC L
                JR .Loop

                endif ; ~ _ASSETS_MANAGER_RELEASE_
