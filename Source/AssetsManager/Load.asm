...
;       временные 256 байт архивного ресурса не помечаются занятыми и после загрузки доступны следующей аллокации
; -----------------------------------------
Load:           LD (GameState.AssetID), A                                       ; сохранение идентификатора загружаемого ассета
                ...
                JR Z, .Allocation                                               ; переход, если флаг сброшен, не требуется автоматическая аллокация
...
.Calculated     PUSH HL                                                         ; сохранение адреса загрузки
                ...
                ; выбрать требуемый диск для загрузки
                PUSH DE                                                         ; сохранение начальной позиции загрузки ресурса
                CALL SelectDisk                                                 ; установить активный привод с требуемым диском
                POP DE                                                          ; восстановление начальной позиции загрузки ресурса
                ...
                BIT ASSETS_NOT_SWITCH_BIT, A                                    ; проверка необходимости установить страницу перед загрузкой
                CALL Z, SetPage                                                 ; установка страницы в 3 банк памяти (#C000-#FFFF)
