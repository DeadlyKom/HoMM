...
JumpHandlerKey: CALL Input.CheckKeyState
                ...
                INC (HL)
                EX DE, HL
                JP (HL)                                                         ; переход, при нажатии клавиши

.IsReleased     OR A
                JR Z, .NotProcessed
                DEC (HL)
                EX DE, HL
                JP (HL)                                                         ; переход, при отжатии клавиши
...
;   если указанная виртуальная клавиша нажата/отжатия то вызовется обрабочик
;   флаг Z соответствет 0/1 состоянию нажатия/отжатия соответственно
;   если обработчик обработал клавишу и не требуется дальнейший проход
;   по виртуальным клавишам, флаг переполнения должен быть сброшен
; Corrupt :
;   HL, DE, BC, AF, AF'
; -----------------------------------------
