                ...
                ifndef _CHARACTER_PATH_DIRECTION_
                define _CHARACTER_PATH_DIRECTION_
; -----------------------------------------
...
; -----------------------------------------
Direction:      ; проверка на перемещение по оси Y, необходимо учитывать чётность оси Y
                LD C, (HL)      ; позиция по оси X точки назначения
                LD A, D
                INC L
                SUB (HL)
                JR Z, .Equal                                                    ; переход, если в одной плоскости
                BIT 0, (HL)     ; проверка чётности строки точки назначения
                JR Z, $+3       ; переход, если чётная
                INC C           ; X += 1 к точки назначения
.Equal          DEC L
                ...
                ; если больше 0 то #01
                ; если меньше 0 то #FF
                JR Z, $+5                                                       ; переход, если результат вычитания равен нулю и преобразование не требуется
                SBC A, A
                OR #01
                ; xxxxxxxx
                LD C, A
...
.DirectionTable ; оси X и Y инвертированы
                ...
                endif ; ~_CHARACTER_PATH_DIRECTION_
