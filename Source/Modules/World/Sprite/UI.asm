...
; Note:
; -----------------------------------------
Load:           ; загрузка графики UI 'стрелок пути'
                SET_PAGE_ASSETS                                                 ; включить страницу расположения ассет менеджера
                LOAD_ASSETS ASSETS_ID_UI_ARROW_PATH_PACK                        ; загрузка ресурса спрайтов "героя"
                ...
                ; инициализация
                LD IX, .Parser
                LD HL, World.Base.Render.Object.UI.Indexes                      ; адрес списока индексов для отображения UI
                LD DE, .HashSequence
