...
Core:           EI
                ...
                SET_TR_DOS_FLAG ENABLE_IM2_BIT                                  ; включить прерывание IM2 по завершении работы с ПЗУ TR-DOS
                ...
                ; -----------------------------------------
                ; установка слота сохранения по умолчанию
                ; -----------------------------------------
                LD A, SAVE_SLOT_0
                LD (GameSession.SaveSlotID), A

                ; ToDo: SaveSlot не должен находиться в текущей структуре, так как это временная структура,
                ;       которая формируется после загрузки или перед сохранением соответственно.
                ;       При вынесении необходимо пересмотреть инициализацию зависимых полей FGameSession
                ; см. FGameSession.
                LD HL, .DefaultSlot
...
.DefaultConfig  FGameConfig {
                    ...
                    VK_Z,                                                       ; клавиша по умолчанию "выбор"
                    ...
                    ; горячие клавиши
                    VK_R,                                                       ; клавиша по умолчанию "книга заклинаний"
                    VK_SPACE,                                                   ; клавиша по умолчанию режима "остановки времени"
                    ;
                    ...
                    2,                                                          ; скорость скролла тайловой карты
                    PLAYBACK_SPEED_DEFAULT                                      ; масштаб проигрывания мирового времени
                }

.DefaultSlot    FSaveSlot {
                    {"Default Slot\0"},                                         ; имя слота сохранения
                    ASSETS_ID_MAP_DEBUG,                                        ; идентификатор карты
                    ; информация игрового времени
                    {
                        ; число "мировых тиков" до наступления следующего часа
                        {
                            LOW WORLD_TICKS_PER_HOUR,
                            HIGH WORLD_TICKS_PER_HOUR
                        },
                        12,                                                     ; часы,         [0..23]     число
                        #01,                                                    ; день месяца,  [#01..#31]  BCD
                        #06,                                                    ; месяц,        [0..11]     число, июль
                        613                                                     ; год,          [0..65535]  число
                    },
                }
