...
SetLine:        XOR A
                ...
                BIT 7, D
                JR Z, .PositiveX
                SET MOVEMENT_STEP_X_NEG_BIT, (IX + FObjectCharacter.Movement.Flags)
...
.PositiveX      BIT 7, H
                JR Z, .PositiveY
                SET MOVEMENT_STEP_Y_NEG_BIT, (IX + FObjectCharacter.Movement.Flags)
...
.PositiveY      PUSH HL
                ...
                SET MOVEMENT_MAJOR_Y_BIT, (IX + FObjectCharacter.Movement.Flags)
                LD (IX + FObjectCharacter.Movement.RemainingSteps), HL
                LD (IX + FObjectCharacter.Movement.PrimaryLength), HL
                SRL H
                RR L
                LD (IX + FObjectCharacter.Movement.LineError), HL
                LD (IX + FObjectCharacter.Movement.SecondaryLength), DE
                RET

.MajorX         LD (IX + FObjectCharacter.Movement.RemainingSteps), DE
                LD (IX + FObjectCharacter.Movement.PrimaryLength), DE
                SRL D
                RR E
                LD (IX + FObjectCharacter.Movement.LineError), DE
                LD (IX + FObjectCharacter.Movement.SecondaryLength), HL
                RET
; -----------------------------------------
...
UpdateEffectiveStepCost:
                ...
                ; расчёт StepCost = ceil(SurfaceStepCost * PathfindingScale / 16)
...
Step:           ; проверка наличия оставшихся DDA-шагов до заданной точки
                LD HL, (IX + FObjectCharacter.Movement.RemainingSteps)
                LD A, L
                OR H
                ...
                BIT MOVEMENT_MAJOR_Y_BIT, (IX + FObjectCharacter.Movement.Flags)
...
.UpdateError    ; определить необходимость шага по вторичной оси
                LD HL, (IX + FObjectCharacter.Movement.LineError)
                LD DE, (IX + FObjectCharacter.Movement.SecondaryLength)
                ...
                LD DE, (IX + FObjectCharacter.Movement.PrimaryLength)
                ADD HL, DE
                LD (IX + FObjectCharacter.Movement.LineError), HL
...
StepX:          ; проверка направления движения по горизонтали
                BIT MOVEMENT_STEP_X_NEG_BIT, (IX + FObjectCharacter.Movement.Flags)
...
StepY:          ; проверка направления движения по вертикали
                BIT MOVEMENT_STEP_Y_NEG_BIT, (IX + FObjectCharacter.Movement.Flags)
