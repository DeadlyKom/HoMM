...
; -----------------------------------------
AddBudget:      ; ToDo: рассчитать итоговую скорость движения персонажа:
                ;   HeroMovementSpeed = MOVEMENT_DEFAULT_SPEED * HeroMovementScale,
                ;   где HeroMovementScale формируется характеристиками, навыками, бафами и дебафами персонажа;
                ;   MovementPending += HeroMovementSpeed * PlaybackScale
                LD DE, MOVEMENT_DEFAULT_SPEED                                   ; базовые единицы движения за один "мировой тик"
                LD A, (GameSession.WorldTimeCtrl + FWorldTimeControl.PlaybackScale)
...
TransferBudget: LD B, A                                                         ; сохранение относительного временного шага
                ...
                ; ToDo: использовать HeroMovementSpeed, рассчитанную по формуле в AddBudget;
                ;   CadenceShare = (HeroMovementSpeed * PlaybackScale) >> (2 - RelativeCadence),
                ;   где RelativeCadence: 0 - четверть, 1 - половина, 2 - весь бюджет cadence-эпохи
                LD HL, MOVEMENT_DEFAULT_SPEED >> 2                              ; четверть полного бюджета текущего масштаба
...
;   IX - адрес структуры объекта (FObjectCharacter)
; Out:
;   FObjectCharacter.HextileID - ID поверхности в текущих координатах объекта
; Corrupt:
;   HL, DE, BC, AF, AF'
; Note:
;   определение поверхности выполняется функцией страницы 1
; -----------------------------------------
UpdateHextileID:
...
;   FObjectCharacter.HextileID - обновлён при переходе в соседнюю координату гекса
; Corrupt:
;   HL, DE, BC, AF, AF'
; Note:
;   Position.X.Low хранит локальное положение в четвертях пикселя;
;   при пересечении границы изменяется Position.X.High и обновляется текущая поверхность
; -----------------------------------------
StepX:          ; проверка направления движения по горизонтали
                ...
                JP UpdateHextileID                                              ; обновить поверхность после перехода
...
;   FObjectCharacter.HextileID - обновлён при переходе в соседнюю строку гексов
; Corrupt:
;   HL, DE, BC, AF, AF'
; Note:
