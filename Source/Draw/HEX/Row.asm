...
; In:
;   IX - адрес рендер буфера (Adr.RenderBuffer + 64) для хранения высоты колонок
;   IY - адрес рендер буфера (Adr.RenderBuffer)
;   DE - адрес строки экрана
;   B  - ширина строки
;   С  - смещение по горизонтали (от начала 0-5)
;   A  - смещение по горизонтали (от 0 до 5)
;          если = 0, гексагон рисуется полностью
;          если > 0, отсечение гексагона слева/справа,
;                    зависит от того какой первый или последний соответственно
;   H  - количество пропускаемых знакомест
; Out:
; Corrupt:
; Note:
...
;
; -----------------------------------------
Row:            ; A - смещение по горизонтали (от 0 до 5)
                ;   если = 0, гексагон рисуется полностью
                ;   если > 0, отсечение гексагона слева (т.к. первый гексагон)

                ; проверка левого отсечения
                OR A
                JR Z, .RowLoop                                                  ; переход, если гексагон рисуется полностью
...
.RowLoop        ; -----------------------------------------
                ...
                ; A - смещение по горизонтали (от 0 до 5)
                ;   если = 0, гексагон рисуется полностью
                ;   если > 0, отсечение гексагона справа (т.к. последний гексагон)
.RightCut       EXX
                LD HL, Column.x8
                LD BC, Column.x6
                LD DE, Column.x2
...
.IncBuffer      INC IYL                                                         ; переход к следующему гексогону
