...
.HShiftTable    EQU $+1
                ...
                INC L
                LD (DE), A                                                      ; первая рисуемая колонка первого гексагона (0-6)
                INC E
...
.Table          ; первый байт, количество рисуемых колонок первого гексагона (1-6), последующие вычисляются быстро
                ...
                ;
                ;  |<---0--->|      |<---1--->|      |<---2--->|      |<---3--->|      |<---4--->|      |<---5--->|
                DB 3, 0, 7, 0,      4, 0, 5, 0,      5, 0, 1, 0,      6, 1, 1, 0,      1, 1, 5, 0,      2, 1, 7, 0      ; 0 смещение по вертикали в знакоместах
                DB 6, 0, 1, 0,      1, 0, 5, 0,      2, 0, 7, 0,      3, 0, 7, 0,      4, 0, 5, 0,      5, 0, 1, 0      ; 1 смещение по вертикали в знакоместах
