...
.RowsLoop       ; расчёт адреса таблицы в зависимости от смещения карты по горизонтали и вертикали
                ; ((номер строки + (FWorldInfo.MapPosition.Y & 0x01) - 1) & 0x01) * 20
                LD A, B
.VShiftTable    EQU $+1
                ADD A, #00
...
.HShiftTable    EQU $+1
                ...
                ADD A, (HL) ; 0 или 1
                INC L
                LD (DE), A                                                      ; смещение обрабатываемой строки в рендер буфере
                INC E

                ; расчёт строки вывода гексагона
                LD A, C     ; номер строки - 1
                ADD A, A    ; x2
                ADD A, A    ; x4
.ShiftVertical  EQU $+1
...
.Table          ; первый байт, количество рисуемых колонок первого гексагона (1-6), последующие вычисляются быстро
                ; второй байт, дополнительное смещение по горизонтали индекса в рендер буфере (0-1)
                ; третий байт, смещение колонки по вертикали в пикселях, 1, 5 или 7
                ; четвёртый байт, резерв
                ;
                ;  |<---0--->|      |<---1--->|      |<---2--->|      |<---3--->|      |<---4--->|      |<---5--->|
                DB 3, 0, 7, 0,      2, 0, 5, 0,      1, 0, 1, 0,      6, 1, 1, 0,      5, 1, 5, 0,      4, 1, 7, 0      ; 0 смещение по вертикали в знакоместах
                DB 6, 0, 1, 0,      5, 0, 5, 0,      4, 0, 7, 0,      3, 0, 7, 0,      2, 0, 5, 0,      1, 0, 1, 0      ; 1 смещение по вертикали в знакоместах
