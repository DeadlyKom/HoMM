...
.Table          ; первый байт, количество пропускаемых колонок первого гексагона (0-5), последующие вычисляются быстро
                ;              7 бит флаг чётности строки
                ; второй байт, дополнительное смещение по горизонтали индекса в рендер буфере (0-1)
                ;
                ; |<-0->|      |<-1->|      |<-2->|      |<-3->|      |<-4->|      |<-5->|
                DB #80+3, 0,    #80+4, 0,    #80+5, 0,    #80+0, 1,    #80+1, 1,    #80+2, 1          ; 0 смещение по вертикали в знакоместах
                DB #00+0, 0,    #00+1, 0,    #00+2, 0,    #00+3, 0,    #00+4, 0,    #00+5, 0          ; 1 смещение по вертикали в знакоместах

                ; проверка размещения всей таблицы внутри одного 256-байтового блока
                if (HexDLGeneration.Table >> 8) - ((HexDLGeneration.Table + 23) >> 8)
                error "whole table must be within single 256 byte block"
                endif

                ; ToDo: для 0 гексагона считается неправильные смещения вперёд (должны быть 5 и 5, а считается 6 и 4 назад и вперёд соответственно!)
