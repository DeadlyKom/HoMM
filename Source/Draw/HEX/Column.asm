...
_2_PIXELS_LINES_ macro
                ...
                endm
; -----------------------------------------
; пропус 8 пиксельных строк знакоместа
; In:
;   SP  - адрес спрайта
;   HL  - адрес экрана
;   DE  - адрес атрибутов
...
PIXEL_MASK_     macro
                ...
                endm
; -----------------------------------------
; пропус 8 пиксельных строк знакоместа с маской
; In:
;   SP  - адрес спрайта
;   HL  - адрес экрана
;   DE  - адрес атрибутов
...
;                 LD (.ContainerSP), SP
;                 LD SP, TemplateSP + 2 * 2                                       ; временный буфер на два значения
...
.x6             CHECK_UPDATE_COUMN                                              ; проверка обновления столбца
                ...
                ; ; переход к 0 строки знакоместа (пикселей)
                ; LD A, H
                ; SUB 5
                ; LD H, A
                ...
                JP .BoundaryUp                                                  ; переход, без проверки верхней границы видимой области
...
.x2             CHECK_UPDATE_COUMN                                              ; проверка обновления столбца
                ...
                ATTR_MASK_SKIP                                                  ; пропус атрибутов знакоместа с маской
                SKIP_HEIGHT_BOUNDARY                                            ; сбор высоты знакоместа (нулевой высоты)
                JR .BoundaryUp                                                  ; переход, без проверки верхней границы видимой области
                ; -----------------------------------------
                ; нижний хвост
                ; -----------------------------------------
...
.x2_            EX AF, AF'
                ...
                ; EX AF, AF'                                                      ; сохранение данных пропускаемых знакомест (значение равно нулю)
...
.BoundaryUp     EX AF, AF'
                JR Z, .DrawNoMask                                                 ; переход, если последующее отображение колонки в видимой области
                DEC A
                EX AF, AF'
                ...
                JR NC, .BoundaryUp                                              ; переход, если флаг рисования без маски
                                                                                ; без проверки верхней границы видимой области
                JR .SkipLoop                                                    ; переход, вывод с маской
...
.SkipLoop       EX AF, AF'                                                      ; восстановление данных пропускаемых знакомест (значение равно нулю)
                JR Z, .MaskUp                                                   ; переход, если последующее отображение колонки в видимой области
                DEC A
                EX AF, AF'
...
.SkipJump       EQU $+1
                ...
                OR A
                JP Z, .Exit                                                     ; переход, если закончились данные спрайта
...
.Loop           EX AF, AF'                                                      ; сохранение высоты последующего вывода с маской
