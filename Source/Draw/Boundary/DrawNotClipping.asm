...
; -----------------------------------------
; пропуск текущего блока составного спрайта
; In:
;   SP+0 - адрес данных текущего блока
;   SP+2 - адрес обработки следующего блока или возврата к вызывающему коду
; Out:
; Corrupt:
;   DE
; Note:
;   вход выполняется переходом без помещения адреса возврата в стек
;   указатель на следующий блок уже обновлён
; -----------------------------------------
DrawNotClipping.DrawDisabled
                ; снятие адреса данных пропущенного блока со стека
                POP DE
                RET                                                             ; продолжение обхода блоков или возврат к вызывающему коду
; -----------------------------------------
...
.BlockAdr       EQU $+1
                ...
                ; проверка флага пропуска рисования блока
                BIT SPRITE_NOT_CLIPPING_SKIP_BIT, A
                JR NZ, DrawNotClipping.DrawDisabled                             ; переход, если пропуск рисования блока включён

                ; сохранение флагов вывода блока для выбора функции рисования
                EX AF, AF'
