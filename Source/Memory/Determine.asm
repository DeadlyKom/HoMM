...
.SetPage_Loop   CALL $
                ...
                ; проверка нумеровоной страницы
                LD A, (HL)
                INC A
                JR NZ, .OccupiedPage                                            ; переход, если страница уже пронумерована
...
;                       https://zxpress.ru/article.php?id=8701 (см)
; -----------------------------------------
...
.Extended       ; копируемый блок кода, дополняющий основную функцию переключения страниц памяти
                ...
                LD A, (HL)
                POP HL
                ; ToDo: из за того что во время переключения страниц через теневое ПЗУ
                ;       TRDOS'а может произойти прерывание, и при возращении из него,
                ;       обратного переключения не произойдёт, т.к. адрес будет не впределах переключения на теневое
                ;       необходимо включить теневые порты, но при работе с TR-DOS'ом их отключать.
                ;
