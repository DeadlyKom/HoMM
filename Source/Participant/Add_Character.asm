...
Add_Character:  ; проверка достижения максимального количества игроков
                ...
                ; маршрут персонажа-человека размещается в одном из первых четырёх фиксированных слотов
                LD A, (IY + FParticipant.Faction.Flags)
                AND COMPUTER
                JR NZ, .Available                                               ; переход, если это ИИ

                ; проверка наличия свободного слота маршрута у участника-человека
                LD A, (IY + FParticipant.CharactersNum)
                CP HERO_PATH_SLOT_COUNT
                RET NC                                                          ; выход, если у участника-человека уже заняты все слоты маршрутов

.Available      PUSH HL                                                         ; сохранение стартовой позиции игрока
                ...
                ; ToDo: все добавления персонажа-человека происходят перед персонажей ИИ,
                ;       тем самым сохранит для персонажей-человека непрерывный диапазон CharacterID [0..3]
...
.Spawn
                ...
                CALL Object.Spawn
                EXX
                ; ToDo: если массив объектов заполнен, Object.Spawn завершится
                ;       с ошибкой после увеличения HeroNum и CharactersNum, а также
                ;       записи CharacterID. в этом случае изменения нужно откатить
